#!/usr/bin/env python3
"""
DenseFormer experiment: Depth-Weighted Average (DWA) after each transformer block.
After each block, compute a learned weighted average of current and ALL past layer
representations. Adds only d(d+3)/2 = 54 params for 9 layers.

Based on: https://arxiv.org/abs/2402.02622
"""
...
class Hyperparameters:
    ...
    xsa_layers: int = int(os.environ.get("XSA_LAYERS", 4))  # apply XSA to last N layers
    vrl: bool = bool(int(os.environ.get("VRL", "1")))  # Value Residual Learning
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,dwa_weights,vrl_lambda",
    ).split(",")
    if pattern
)
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        if use_vrl:
            ...
            # Init: vrl_lambda = [0.0, 1.0] so identity (use current V only) at start
            self.vrl_lambda = mx.array([0.0, 1.0], dtype=mx.float32)

    def __call__(self, x: mx.array, v_first: mx.array | None = None) -> tuple[mx.array, mx.array]:
        bsz, seqlen, dim = x.shape
        q = self.c_q(x).reshape(bsz, seqlen, self.num_heads, self.head_dim).transpose(0, 2, 1, 3)
        k = self.c_k(x).reshape(bsz, seqlen, self.num_kv_heads, self.head_dim).transpose(0, 2, 1, 3)
        v = self.c_v(x).reshape(bsz, seqlen, self.num_kv_heads, self.head_dim).transpose(0, 2, 1, 3)

        # VRL: blend first layer's V into current V
        if self.use_vrl and v_first is not None:
            lam = mx.softmax(self.vrl_lambda)
            v = lam[0] * v_first + lam[1] * v
        ...
        if self.use_xsa:
            ...
            v_expanded = mx.repeat(v, self.num_heads // self.num_kv_heads, axis=1)
            dot = mx.sum(y * v_expanded, axis=-1, keepdims=True)
            v_norm_sq = mx.sum(v_expanded * v_expanded, axis=-1, keepdims=True) + 1e-6
...
class Block(nn.Module):
    ...
    def __call__(self, x: mx.array, x0: mx.array, v_first: mx.array | None = None) -> tuple[mx.array, mx.array]:
        mix = self.resid_mix.astype(x.dtype)
        x = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
        attn_out, v = self.attn(self.attn_norm(x), v_first=v_first)
...
class GPT(nn.Module):
    def __init__(self, vocab_size: int, num_layers: int, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int,
                 ...
                 qk_gain_init: float, xsa_layers: int = 4, use_vrl: bool = False):
        super().__init__()
        ...
        # DenseFormer: DWA weights — alpha[i] has (i+2) entries (for embedding + layers 0..i)
        # Initialized so alpha[i][i+1] = 1 (identity: just use current layer output)
        # and all others = 0. This means DWA is a no-op at init.
        self.num_layers = num_layers
        self.dwa_weights = [
            mx.array([0.0] * (i + 1) + [1.0], dtype=mx.float32)
            for i in range(num_layers)
        ]
    ...
    def __call__(self, input_ids: mx.array) -> mx.array:
        ...
        # Store all layer outputs for DWA (index 0 = embedding output)
        layer_outputs = [x]

        for i in range(self.num_encoder_layers):
            x, v = self.blocks[i](x, x0, v_first=v_first)
            # Capture v from layer 0 for VRL
            if i == 0 and self.use_vrl:
                v_first = v
            # DWA: weighted sum of all outputs so far (embedding + layers 0..i)
            layer_outputs.append(x)
            w = mx.softmax(self.dwa_weights[i])
            x = sum(w[j] * layer_outputs[j] for j in range(i + 2))
            layer_outputs[-1] = x  # update stored output to DWA-mixed version
            skips.append(x)

        for i in range(self.num_decoder_layers):
            if skips:
                x = x + self.skip_weights[i].astype(x.dtype)[None, None, :] * skips.pop()
            x, v = self.blocks[self.num_encoder_layers + i](x, x0, v_first=v_first)
            # DWA for decoder layers too
            layer_idx = self.num_encoder_layers + i
            layer_outputs.append(x)
            w = mx.softmax(self.dwa_weights[layer_idx])
            x = sum(w[j] * layer_outputs[j] for j in range(layer_idx + 2))
            layer_outputs[-1] = x
...
class SplitOptimizers:
    def __init__(self, model: GPT, args: Hyperparameters):
        ...
        self.scalar_keys = [
            k
            for k, p in params.items()
            if k == "skip_weights" or k.startswith("dwa_weights.") or (k.startswith("blocks.") and (p.ndim < 2 or any(pattern in k for pattern in CONTROL_TENSOR_NAME_PATTERNS)))
        ]
...
def main() -> None:
    ...
    # Log DWA param count
    dwa_params = sum(len(w) for w in model.dwa_weights)
    log(f"denseformer:dwa_params={dwa_params}")
    xsa_block_ids = [i for i, b in enumerate(model.blocks) if b.attn.use_xsa]
    vrl_block_ids = [i for i, b in enumerate(model.blocks) if b.attn.use_vrl]
    log(f"vrl:enabled={args.vrl},layers={vrl_block_ids}")
    log(f"xsa:layers={args.xsa_layers} blocks={xsa_block_ids}")

...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,dwa_weights",
    ).split(",")
    if pattern
)
...
class GPT(nn.Module):
    def __init__(
        ...
        # Initialized so alpha[i][i+1] = 1 (identity) and all others = 0.
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            # DWA: weighted sum of all outputs so far
            layer_outputs.append(x)
            w = F.softmax(self.dwa_weights[i].to(dtype=x.dtype), dim=0)
        ...
        for i in range(self.num_decoder_layers):
            ...
            layer_outputs.append(x)
            w = F.softmax(self.dwa_weights[layer_idx].to(dtype=x.dtype), dim=0)
...
def main() -> None:
    ...
    # - vectors/scalars/skip_weights/dwa_weights use SCALAR_LR via Adam
    ...
    for dwa_w in base_model.dwa_weights:
        scalar_params.append(dwa_w)

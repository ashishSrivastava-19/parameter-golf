...
class NgramCache:
    ...
    def update(self, context: tuple[int, ...], token: int) -> None:
        """Record token following context for all n-gram orders."""
    ...
    with torch.no_grad():
        for start in range(0, n_seqs, chunk_size):
            ...
            batch_in = torch.stack([
                val_tokens[i * seq_len: i * seq_len + seq_len].to(dtype=torch.int64)
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, v_first: Tensor | None = None) -> tuple[Tensor, Tensor]:
        ...
        if self.use_vrl and v_first is not None:
            lam = F.softmax(self.vrl_lambda, dim=0)
...
class GPT(nn.Module):
    def __init__(
        ...
        self.dwa_weights = nn.ParameterList([
            nn.Parameter(torch.tensor([0.0] * (i + 1) + [1.0], dtype=torch.float32))
            for i in range(num_layers)
        ])
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            w = F.softmax(self.dwa_weights[i], dim=0)
        ...
        for i in range(self.num_decoder_layers):
            ...
            layer_outputs.append(x)
            w = F.softmax(self.dwa_weights[layer_idx], dim=0)
    ...
    def forward_logits(self, input_ids: Tensor) -> Tensor:
        """Same as forward() but returns logits [B, T, vocab] instead of cross-entropy loss."""
        x = self.tok_emb(input_ids)
        x = F.rms_norm(x, (x.size(-1),))
        x0 = x
        skips: list[Tensor] = []
        ...
        if self.tie_embeddings:
            ...
            if self.lm_head is None:
                raise RuntimeError("lm_head is required when tie_embeddings=False")
            logits_proj = self.lm_head(x)
        return self.logit_softcap * torch.tanh(logits_proj / self.logit_softcap)  # [B, T, vocab]
...
def main() -> None:
    ...
    # Log DenseFormer + VRL config
    dwa_params = sum(p.numel() for p in base_model.dwa_weights)
    ...
    # DWA weights go to scalar optimizer
    for dwa_p in base_model.dwa_weights:
        scalar_params.append(dwa_p)
    ...
    # N-gram cache eval (single-pass causal — fully legal per organizer ruling Mar 23)
    if master_process:
        log0("starting ngram_cache_eval ...")
        t_ng = time.perf_counter()
        ng_loss, ng_bpb = eval_val_ngram(
            args,
            base_model,
            device,
            val_tokens,
        ...
        log0(
            f"ngram_cache_eval val_loss:{ng_loss:.4f} val_bpb:{ng_bpb:.4f} "
            f"eval_time:{1000.0 * (time.perf_counter() - t_ng):.0f}ms"
        )
        log0(f"ngram_cache_eval_exact val_loss:{ng_loss:.8f} val_bpb:{ng_bpb:.8f}")
